...
const useMusicStore = defineStore('music', {
    ...
    actions: {
        async getMusicUrl(id: number) {
            ...
            // this.musicUrl = await axios.get(`https://www.byfuns.top/api/1/?id=${id}`)
            // return Promise.resolve(this.musicUrl);
            // console.log(this.musicUrl);

            try {
                const result = await axios.get(`https://www.hhlqilongzhu.cn/api/dg_wyymusic.php?id=${id}&type=json`)
                this.musicUrl = result.data.music_url;
